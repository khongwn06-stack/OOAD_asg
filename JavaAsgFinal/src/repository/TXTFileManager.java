...
public class TXTFileManager implements FileManager {
    ...
    public void saveData(Object data, String fileName) throws FileProcessingException {
        ...
        // -----------------------------------------------------
        // Route
        if (fileName.equalsIgnoreCase("routes.txt"))
        {
            try
            {
                ...
                try (PrintWriter outputFile = new PrintWriter(fileName))
                {
                    ...
                    {
                        outputFile.println(route.toFileString());
                    }
                }
            ...
            {
                throw new FileProcessingException("Unable to save data to " + fileName, e);
            }
            catch (ClassCastException e)
            {
                throw new FileProcessingException("Invalid route data for " + fileName, e);
            }
    ...
    public static void saveTicket(Ticket ticket)
    {
        if (ticket == null)
        {
            System.out.println("Cannot save null ticket.");
            return;
        }
        ...
        if (file.exists())
        {
        	try (Scanner inputFile = new Scanner(file)){
                while (inputFile.hasNextLine()){
                    ...
                    if (data.length > 0 && data[0].equalsIgnoreCase(ticket.getTicketId()))
                    {
                ...
                System.out.println("Error reading tickets.");
                return;
            }
        }
        ...
        try
        {
            ...
            System.out.println("Ticket data saved successfully.");
